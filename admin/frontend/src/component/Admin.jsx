...
function Admin() {
  ...
  let [onhandle, setOnHandle] = useState({ email: "", pass: "" });
  const onChangeFun = (e) => {
    let oldval = { ...onhandle };
  ...
  const formHandle = async (e) => {
    ...
    const response = await fetch("http://localhost:3000/admin/login/", {
      ...
      body: JSON.stringify(onhandle),
    });
    const result = await response.json();
    console.log(result);
    if (result.token) {
      localStorage.setItem("a_token", result.token);
      localStorage.setItem("a_email", result.email);
      navigate("/");
    } else {
